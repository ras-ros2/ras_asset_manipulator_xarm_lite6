...
def generate_ros2_control_params_temp_file(ros2_control_params_path, prefix='', add_gripper=False, add_bio_gripper=False, ros_namespace='', update_rate=None, robot_type='xarm'):
    if ros_namespace or prefix or add_gripper or add_bio_gripper or update_rate:
        ...
        if add_gripper:
            gripper_control_params_path = os.path.join(get_package_share_directory('xarm_controller_sim'), 'config', '{}_gripper_controllers.yaml'.format(robot_type))
            ...
            gripper_control_params_path = os.path.join(get_package_share_directory('xarm_controller_sim'), 'config', 'bio_gripper_controllers.yaml')
            # check file is exists or not
            if os.path.exists(gripper_control_params_path):
                with open(gripper_control_params_path, 'r') as f:
                    gripper_control_params_yaml = yaml.safe_load(f)
...
def generate_dual_ros2_control_params_temp_file(
    ...
    if add_gripper_1:
        gripper_control_params_path = os.path.join(get_package_share_directory('xarm_controller_sim'), 'config', '{}_gripper_controllers.yaml'.format(robot_type_1))
        # check file is exists or not
        if os.path.exists(gripper_control_params_path):
            with open(gripper_control_params_path, 'r') as f:
                gripper_control_params_yaml = yaml.safe_load(f)
    ...
    if add_gripper_2:
        gripper_control_params_path = os.path.join(get_package_share_directory('xarm_controller_sim'), 'config', '{}_gripper_controllers.yaml'.format(robot_type_2))
        # check file is exists or not
        if os.path.exists(gripper_control_params_path):
            with open(gripper_control_params_path, 'r') as f:
                gripper_control_params_yaml = yaml.safe_load(f)
