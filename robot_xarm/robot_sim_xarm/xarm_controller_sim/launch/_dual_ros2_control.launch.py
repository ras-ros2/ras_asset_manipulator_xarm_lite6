...
def launch_setup(context, *args, **kwargs):
    ...
    # ros2 control params
    # xarm_controller_sim/launch/lib/robot_controller_lib.py
    mod = load_python_launch_file_as_module(os.path.join(get_package_share_directory('xarm_controller_sim'), 'launch', 'lib', 'robot_controller_lib.py'))
    ...
    ros2_control_params = generate_dual_ros2_control_params_temp_file(
        os.path.join(get_package_share_directory('xarm_controller_sim'), 'config', '{}{}_controllers.yaml'.format(robot_type_1.perform(context), dof_1.perform(context) if robot_type_1.perform(context) in ('xarm', 'lite') else '')),
        os.path.join(get_package_share_directory('xarm_controller_sim'), 'config', '{}{}_controllers.yaml'.format(robot_type_2.perform(context), dof_2.perform(context) if robot_type_2.perform(context) in ('xarm', 'lite') else '')),
    ...
    # robot_description
    # xarm_description_sim/launch/lib/robot_description_lib.py
    xacro_file = PathJoinSubstitution([FindPackageShare('xarm_description_sim'), 'urdf', 'dual_xarm_device.urdf.xacro'])
    mod = load_python_launch_file_as_module(os.path.join(get_package_share_directory('xarm_description_sim'), 'launch', 'lib', 'robot_description_lib.py'))
